...
public class CubeHands : MonoBehaviour
{
    /// <summary>
    ...
    void Start()
    {
        ...
        m_hands = new OVRHand[]
        {
            ...
            GameObject.Find("OVRCameraRig/TrackingSpace/LeftHandAnchor/LeftControllerAnchor/OVRHandPrefab").GetComponent<OVRHand>(),
            GameObject.Find("OVRCameraRig/TrackingSpace/RightHandAnchor/RightControllerAnchor/OVRHandPrefab").GetComponent<OVRHand>()
        };
    ...
    void Update()
    {
        //check for Pinky finger pinch on the left hand, and make che cube red in this case
        // plus move it along -z
        if (m_hands[0].GetFingerIsPinching(OVRHand.HandFinger.Pinky))
        {
            m_renderer.material.color = Color.red;
            transform.position = transform.position - new Vector3(0.0f, 0.0f, Time.deltaTime * 0.5f);
        }
        else if (m_hands[1].GetFingerIsPinching(OVRHand.HandFinger.Pinky))
        {
            m_renderer.material.color = Color.cyan;

            float force = m_hands[1].GetFingerPinchStrength(OVRHand.HandFinger.Pinky);
            transform.position = transform.position + new Vector3(0.0f, Time.deltaTime * 0.25f * force, 0.0f);
        }
        //if no pinch, and the cube was red, make it white again
        else if ((m_renderer.material.color == Color.red) || ((m_renderer.material.color == Color.cyan)))
